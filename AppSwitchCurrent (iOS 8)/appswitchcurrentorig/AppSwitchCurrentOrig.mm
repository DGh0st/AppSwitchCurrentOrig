...
@interface PSListController (AppSwitchCurrentOrig)
- (void)presentViewController:(UIViewController *)viewControllerToPresent animated:(BOOL)flag completion:(void (^)(void))completion;
- (void)dismissViewControllerAnimated:(BOOL)flag completion:(void (^)(void))completion;
- (UINavigationController*)navigationController;
@end

@interface AppSwitchCurrentOrigListController: PSListController <MFMailComposeViewControllerDelegate>{
}
@end

@implementation AppSwitchCurrentOrigListController
- (id)specifiers {
	if(_specifiers == nil) {
		_specifiers = [[self loadSpecifiersFromPlistName:@"AppSwitchCurrentOrig" target:self] retain];
	}
	return _specifiers;
}
-(void)email{
	if([MFMailComposeViewController canSendMail]){
		...
		[email setSubject:@"AppSwitchCurrentOrig Support"];
		[email setToRecipients:[NSArray arrayWithObjects:@"deeppwnage@yahoo.com", nil]];
		[email addAttachmentData:[NSData dataWithContentsOfFile:@"/var/mobile/Library/Preferences/com.dgh0st.appswitchcurrentOrig.plist"] mimeType:@"application/xml" fileName:@"Prefs.plist"];
